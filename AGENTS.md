...
## ディレクトリ構成
- `src/main/` — アプリ本体 ＋ PWA。**配信/デプロイ対象（これだけを web ルートとして配信する）**。
- `src/test/` — ヘッドレステスト。**PWA には含めない**（web ルート外なので Service Worker もキャッシュしない）。
...
## 動作確認
- ローカル表示: `tools/serve.sh [PORT]`（src/main を HTTP 配信しブラウザを開く。停止は Ctrl-C）。
  中身は `python3 -m http.server --directory src/main` 相当。
