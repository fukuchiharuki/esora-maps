...
- **左側通行**。
- **タップは限定インタラクションのみ**：路肩のゴミをタップするとゴミ収集車がそのゴミへ向かう（不在ならスポーン）。それ以外のタップは何も起こさない。**詳細表示・投稿・評価・検索・ログイン・現在地・外部遷移は引き続き禁止**。タップが「消費」されなければダブルタップズームは従来どおり。
...
- litter.js — 路肩のゴミ（車両以外のワールドオブジェクト。生成/回収/ヒットテスト）
- camera.js — ビュー変換・ズーム・画面↔ワールド変換
- render.js — 描画（道路・車両・ゴミ ＋ カーチェイス発生アイコン右下 HUD。引きで車両/ゴミを拡大）
- input.js — ドラッグ / ピンチ / ホイール / ダブルタップ / タップ（onTap）
- scenario.js — ScenarioEvent 基盤 ＋ カーチェイス（逃走車 vs パトカー）＋ ゴミ収集（収集車 vs ゴミ。タップ誘導）
...
- ヘッドレステスト（自己完結・依存なし。どのシードでも通る）:
  ...
  路肩退避・ゴミ収集（接近で寄せ回収・操作なしに探索しない・タップ誘導/スポーン）・描画を検証。
