...
import { vehicles, makeVehicle, removeVehicle, repositionVehicle } from './vehicles.js';
...
const CAPTURE_HOLD = 0.2;       // この秒数だけ接近を維持したら確保確定 (一瞬のすれ違い誤検出を防ぐ)

// 確保: その場で停止させ、以後動かさない。役割・見た目はそのまま (パトカーは点灯継続)。
// parked にしてグリッドロック撤去の対象外とし、通常車と同じくスクロールでのみデスポーンさせる。
function capture(v) {
  v.steer = null;
  v.speed = 0;
  v.vmax = 0;
...
// 確保可能か: 交差点の外で、パトカーが逃走車を「同方向に・至近距離まで」詰めているとき。
// 交差点内では確保しない (予約ロックを握ったまま固定されると詰まる / 重なるのを避ける)。
function canCapture(police, flee) {
  ...
  if (ftile.junction || ptile.junction) return false;
...
function spawnChase(now) {
  ...
  police.juncCap = CHASE_JUNC_CAP; police.curveCap = CHASE_CURVE_CAP;
  police.accel = CHASE_ACCEL; police.decel = CHASE_DECEL;
