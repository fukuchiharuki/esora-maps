...
function spawnPolice(flee) {
  ...
  for (const { t, din, dout } of pick) {
    ...
    // パトカーは逃走車 (vmax 50) の 2 倍 (vmax 100) で明確に速い → 画面外から追いつき、減速地点
    // (カーブ/渋滞/交差点手前) で背後に詰めて確保しうる。直線では追従減速で車間が開くので逃げ切りも残る。
    p.role = 'police'; p.color = '#eaf0f7'; p.vmax = 100;
...
const COLLECT_RUSH_VMAX = COLLECT_DRIVE_VMAX * 1.5; // タップされた目的地へ向かう間は急ぐ (通常の 1.5 倍)
    ...
    update(dt, t) {
      ...
      if (this.dest && pool.items.indexOf(this.dest) >= 0) {
        ...
        if (destReachable(truck, this.dest)) {
          ...
          returnToLane(truck);
          truck.vmax = COLLECT_RUSH_VMAX; // タップされた目的地へ向かう間は急ぐ (通常の 1.5 倍)
          this.asideT = 0;
        }
      } else {
